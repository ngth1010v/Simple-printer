...
namespace {
    ...
    COLORREF Darken(COLORREF c, int amount) {
        int r = std::max(0, (int)GetRValue(c) - amount);
        int g = std::max(0, (int)GetGValue(c) - amount);
        int b = std::max(0, (int)GetBValue(c) - amount);
        return RGB(r, g, b);
    }
    ...
    bool ParseHexByte(const std::string& s, int& value) {
    ...
    bool ParseRgbHex(const std::string& s, COLORREF& out) {
        std::string t = s;
        if (!t.empty() && t[0] == '#') t.erase(t.begin());
        if (t.size() != 6) return false;

        int r = 0, g = 0, b = 0;
        if (!ParseHexByte(t.substr(0, 2), r)) return false;
        if (!ParseHexByte(t.substr(2, 2), g)) return false;
        if (!ParseHexByte(t.substr(4, 2), b)) return false;
...
int FileListView::GetListTop() const {
    return Layout::SECTION_MARGIN + Layout::LABEL_H + Layout::INPUT_GAP;
}

int FileListView::GetButtonsTop(int clientH) const {
    return clientH - Layout::SECTION_MARGIN - Layout::INPUT_H;
}

int FileListView::GetListBottom(int clientH) const {
    return GetButtonsTop(clientH) - Layout::INPUT_GAP;
}

int FileListView::GetListViewportHeight(int clientH) const {
    return std::max(0, GetListBottom(clientH) - GetListTop());
}
...
RECT FileListView::GetRowRect(int index) const {
    ...
    const int left = Layout::SECTION_MARGIN;
    const int right = std::max((long)left, rcClient.right - Layout::SECTION_MARGIN);

    const int rowTop = GetListTop() + index * GetRowStep() - m_scrollY;
...
bool FileListView::HitTestButtonsArea(int x, int y) const {
    RECT rc = GetClientRectSafe();
    const int buttonsTop = GetButtonsTop(rc.bottom);
    return y >= buttonsTop && y < (buttonsTop + Layout::INPUT_H);
}
...
int FileListView::HitTestRow(int x, int y) const {
    ...
    const int listBottom = GetListBottom(rcClient.bottom);
    if (y < listTop || y >= listBottom) return -1;

    const int step = GetRowStep();
    const int contentY = y + m_scrollY - listTop;
    const int index = contentY / step;
...
void FileListView::UpdateScrollBar() {
    ...
    RECT rc = GetClientRectSafe();
    const int viewportH = GetListViewportHeight(rc.bottom);

    const int rowCount = (int)m_files.size();
    const int contentH = std::max(0, rowCount * Layout::FilesSection::FileListView::ROW_H +
...
void FileListView::EnsureSelectionVisible() {
    ...
    RECT rc = GetClientRectSafe();
    const int top = GetListTop();
    const int bottom = GetListBottom(rc.bottom);

    const int rowTop = top + m_selectedIndex * GetRowStep() - m_scrollY;
...
void FileListView::CreateSelectedEdits() {
    ...
    if (m_editFrom) {
        ...
        SetWindowPos(m_editFrom, nullptr, 0, 0, 0, 0, SWP_FRAMECHANGED | SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);
    ...
    if (m_editTo) {
        ...
        SetWindowPos(m_editTo, nullptr, 0, 0, 0, 0, SWP_FRAMECHANGED | SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);
...
void FileListView::UpdateLayout() {
    ...
    const int btnY = GetButtonsTop(clientH);
    const int btnW = std::max(0, (clientW - 2 * Layout::SECTION_MARGIN - 3 * Layout::INPUT_GAP) / 4);

    const int x0 = Layout::SECTION_MARGIN;
    const int gap = Layout::INPUT_GAP;

    if (m_btnUp) {
        MoveWindow(m_btnUp, x0, btnY, btnW, Layout::INPUT_H, TRUE);
    }
    if (m_btnDown) {
        MoveWindow(m_btnDown, x0 + btnW + gap, btnY, btnW, Layout::INPUT_H, TRUE);
    }
    if (m_btnRemove) {
        MoveWindow(m_btnRemove, x0 + (btnW + gap) * 2, btnY, btnW, Layout::INPUT_H, TRUE);
    }
    if (m_btnAdd) {
        MoveWindow(m_btnAdd, x0 + (btnW + gap) * 3, btnY, btnW, Layout::INPUT_H, TRUE);
    }
...
void FileListView::OnLButtonDownInternal(int x, int y) {
    ...
    // Click on empty area => exit selection.
    ClearSelection();
    SetFocus(m_hwnd);
}
...
void FileListView::OnVScrollInternal(WPARAM wParam) {
    ...
    case SB_LINEDOWN:
        ...
        break;
    case SB_PAGEUP:
        ScrollTo(m_scrollY - GetListViewportHeight(GetClientRectSafe().bottom));
        break;
    case SB_PAGEDOWN:
        ScrollTo(m_scrollY + GetListViewportHeight(GetClientRectSafe().bottom));
        break;
...
void FileListView::OnCommandInternal(WPARAM wParam, LPARAM lParam) {
    ...
    if ((id == ID_FLV_EDIT_FROM || id == ID_FLV_EDIT_TO) && hwndFrom) {
        if (code == EN_KILLFOCUS) {
            // Commit whatever user typed.
            CommitSelectedEditsIfNeeded(false);
            return;
        }

        if (code == EN_CHANGE) {
            // Only commit on blur / enter, no data mutation here.
            return;
        }
    }

    if (id == ID_FLV_BTN_UP && code == BN_CLICKED) {
        if (m_selectedIndex >= 0 && m_selectedIndex < (int)m_files.size() && m_cbMoveUp) {
            m_cbMoveUp(m_files[m_selectedIndex].path);
        }
        return;
    }

    if (id == ID_FLV_BTN_DOWN && code == BN_CLICKED) {
        if (m_selectedIndex >= 0 && m_selectedIndex < (int)m_files.size() && m_cbMoveDown) {
            m_cbMoveDown(m_files[m_selectedIndex].path);
        }
        return;
    }

    if (id == ID_FLV_BTN_REMOVE && code == BN_CLICKED) {
        if (m_selectedIndex >= 0 && m_selectedIndex < (int)m_files.size() && m_cbRemove) {
            m_cbRemove(m_files[m_selectedIndex].path);
        }
        return;
    }

    if (id == ID_FLV_BTN_ADD && code == BN_CLICKED) {
        if (m_cbAdd) {
            const std::string path = (m_selectedIndex >= 0 && m_selectedIndex < (int)m_files.size())
                ? m_files[m_selectedIndex].path
                : std::string{};
            m_cbAdd(path);
        }
        return;
...
void FileListView::OnDrawItemInternal(const DRAWITEMSTRUCT* dis) {
...
void FileListView::OnCtlColorEditInternal(HDC hdc, HWND hwnd) {
    SetBkMode(hdc, TRANSPARENT);
...
void FileListView::DrawRow(HDC hdc, int index, const RECT& clipRc) {
    ...
    RECT divTextRc = divRc;
    DrawCenteredText(hdc, divTextRc, L"-", Style::INPUT_TEXT, m_font);
}
...
void FileListView::OnPaintInternal(HDC hdc) {
    ...
    // Outer border
    RECT sec = rcClient;
    if (sec.right > 0 && sec.bottom > 0) {
        DrawRoundedFrame(hdc, sec, Style::SECTION_BORDER, 0);
    }
    ...
    // Title
    RECT titleRc{
        Layout::SECTION_MARGIN,
        Layout::SECTION_MARGIN,
        rcClient.right - Layout::SECTION_MARGIN,
        Layout::SECTION_MARGIN + Layout::LABEL_H
    };
    ...
    RECT listClip{
        Layout::SECTION_MARGIN,
        listTop,
        std::max((long)Layout::SECTION_MARGIN, rcClient.right - Layout::SECTION_MARGIN),
        std::max(listTop, listBottom)
    };

    if (listClip.bottom > listClip.top) {
        int step = GetRowStep();
        int startY = std::max(0, m_scrollY - LOAD_OUTSIDE_PX);
        int endY = m_scrollY + (listClip.bottom - listClip.top) + LOAD_OUTSIDE_PX;

        int first = std::max(0, startY / step - 1);
        int last = std::min((int)m_files.size() - 1, endY / step + 1);

        for (int i = first; i <= last; ++i) {
            DrawRow(hdc, i, listClip);
        }
    }
...
void FileListView::Resize(int parentWidth, int parentHeight) {
    ...
    MoveWindow(m_hwnd, x, y, std::max(0, w), std::max(0, h), TRUE);
    UpdateLayout();
}
...
LRESULT CALLBACK FileListView::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_DRAWITEM:
        if (self) {
            self->OnDrawItemInternal(reinterpret_cast<const DRAWITEMSTRUCT*>(lParam));
        }
        return 0;
